function [MU,RHO,F,m] = P_CVMCTBN_STAR_POST_REG_DIMS(node,dt,M,t0,Z,TZ,thresh)
...
for m=1:M-1
    ...
    for i=1:L
        ...
        [q_am,q_gm] = P_CVMCTBN_EFF_RATES_SPARSE_REG_DIMS(node,i,m,MU);
        
        if m>1
             %psi0 = P_CVMCTBN_COUP_D(node,i,squeeze(mu(m,:,:,:)),squeeze(rho(m-1,:,:,:)),D);
            % psi0 = P_CVMCTBN_COUP_DIMS(node,i,m,MU,RHO);
            [psi0] = P_CVMCTBN_COUP_SPARSE_REG_DIMS(node,i,m,MU,RHO);
        end
