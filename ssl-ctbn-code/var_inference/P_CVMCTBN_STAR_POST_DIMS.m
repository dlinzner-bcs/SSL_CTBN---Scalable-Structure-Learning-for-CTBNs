function [MU,RHO,F,m] = P_CVMCTBN_STAR_POST_DIMS(node,dt,M,t0,Z,TZ,thresh)
...
for m=1:M-1
    ...
    for i=1:L
        ...
        rho=RHO{m}{i};
        D=node(i).D;
        [q,~] = P_CVMCTBN_EFF_RATES_DIMS(node,i,m,MU);
        psi0=zeros(tau,D);
        if m>1
        ...
        [Tt Y] = ode15s(@(t,y) CVM_CTBN_rho_fastD(D,t, y, xt, q, xt, psi0), TSPAN_Y, rhoT');
        R{length(TZ)+1}=Y;
        
        for k=length(TZ):-1:1
            Y_mem=Y;
            rhoT=Y(end,:).*(squeeze(Z{i}(:,k))');
            if k>1
            ...
            if isempty(msglast)==0
                ...
                %options = odeset('Jacobian',@(t,y)J_CVM_CTBN_mu(D,t, xt, q(1:tau,:), xt, squeeze(rho(m,1:tau,i,:))));
               [~,Y] = ode15s(@(t,y) CVM_CTBN_rho_fastD(D,t, y, xt, q, xt, psi0), TSPAN_Y, rhoT');
                R{k}=Y;
            end
        ...
        [~, Y] = ode15s(@(t,y) CVM_CTBN_mu_fastD(t, y,D, xt, q(1:tau,:,:), xt, rho(1:tau,:)), TSPAN_M, squeeze(mu(1,:)));
