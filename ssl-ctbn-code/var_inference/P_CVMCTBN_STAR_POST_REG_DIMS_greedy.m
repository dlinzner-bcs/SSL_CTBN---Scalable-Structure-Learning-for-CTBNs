function [MU,RHO,F,m] = P_CVMCTBN_STAR_POST_REG_DIMS_greedy(node,dt,M,t0,Z,TZ,thresh)
...
for m=1:M-1
    ...
    for i=1:L
        ...
        [q_am] = P_CVMCTBN_EFF_RATES_SPARSE_REG_DIMS_greedy(node,i,m,MU);
        ...
        for k=length(TZ):-1:1
            ...
            if isempty(msglast)==0
                ...
                %options = odeset('Jacobian',@(t,y)J_CVM_CTBN_mu(D,t, xt, q(1:tau,:), xt, squeeze(rho(m,1:tau,i,:))));
                [Tt Y] = ode15s(@(t,y) CVM_CTBN_rho_fast_sparse_reg_DIMS(D,t, y, xt, q_am, q_am, psi0), TSPAN_Y, rhoT');
                R{k}=Y;
            end
        end
        for d=1:D
       ...
       % [Tt Y] = ode15s(@(t,y) CVM_CTBN_mu_fastD(D,t, y, xt, q(1:tau,:,:), xt, RHO{m}{i}, TSPAN_M, mu(m,1,i,:)));
         [~, Y] = ode15s(@(t,y) CVM_CTBN_mu_fastD(t, y,D, xt, q_am(1:tau,:,:), xt, rho(1:tau,:)), TSPAN_M, squeeze(mu(1,:)));
