function [MU,RHO,node] = ctbn_expectation_sparse_reg_par_DIMS_greedy(node,dt,Mmax,t0,Z,TZ,thresh)
...
parfor k=1:length(TZ)
    try
        [mu,rho,~] = P_CVMCTBN_STAR_POST_REG_DIMS_greedy(node,dt,Mmax,t0,Z{k},TZ{k},thresh);
        MU{k}=mu{Mmax};
        RHO{k}=rho{Mmax-1};
        TK=linspace(0,TZ{k}(end),ceil(TZ{k}(end)/dt));
        for i=1:length(node)
            [T0,M0] = CTBN_cond_stat_star_sparse_reg_DIMS_greedy(node,i,length(TK)-2,dt,MU{k},RHO{k});
            for l=1:length(node(i).pi)
